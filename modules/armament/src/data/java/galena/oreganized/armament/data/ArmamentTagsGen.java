...
public class ArmamentTagsGen {

    public ArmamentTagsGen() {
    ...
    private void blocks(RegistrateTagsProvider.IntrinsicImpl<Block> provider) {
        ...
        provider.addTag(blockTag("supplementaries", "bomb_breakable")).add(ArmamentBlocks.SHRAPNEL_BOMB.getKey());
        provider.addTag(blockTag("supplementaries", "cannon_tnts")).add(ArmamentBlocks.SHRAPNEL_BOMB.getKey());
    }

    private void entities(RegistrateTagsProvider.IntrinsicImpl<EntityType<?>> provider) {
        provider.addTag(ArmamentTags.Entities.BOLT_SUSCEPTIBLE)
                .add(EntityType.IRON_GOLEM)
                .addOptional(ResourceLocation.fromNamespaceAndPath("caverns_and_chasms", "copper_golem"));

        provider.addTag(ArmamentTags.Entities.BOLT_RESISTANT)
