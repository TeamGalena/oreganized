...
public class ODatagen {
    ...
    static {
        REGISTRATE.getDataGenInitializer().addDependency(ProviderType.DYNAMIC, ODataPacks.PROVIDER);
        REGISTRATE.getDataGenInitializer().addDependency(DAMAGE_TYPE_TAGS, ODataPacks.PROVIDER);
        REGISTRATE.getDataGenInitializer().addDependency(TRIM_MATERIAL_TAGS, ODataPacks.PROVIDER);
    }
    ...
    public static <T> void addDataRegistryEntries(ResourceKey<Registry<T>> registry, RegistrySetBuilder.RegistryBootstrap<T> bootstrap, Consumer<BiConsumer<ResourceKey<?>, ICondition>> conditions) {
        ...
        REGISTRATE.addDataGenerator(ODataPacks.PROVIDER, conditions::accept);
    }
